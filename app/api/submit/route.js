    ...
    try {
        ...
        if (!process.env.GOOGLE_CLIENT_EMAIL) {
            return new Response(
                JSON.stringify({
                    status: "error",
                    message: "Google Sheets API client email is not configured.",
                }),
                {status: 500}
            );
        }
        ...
        if (!process.env.GOOGLE_PRIVATE_KEY) {
            return new Response(
                JSON.stringify({
                    status: "error",
                    message: "Google Sheets API private key is not configured.",
                }),
                {status: 500}
            );
        }
        ...
        if (!process.env.GOOGLE_SHEET_ID) {
            return new Response(
                JSON.stringify({
                    status: "error",
                    message: "Google Sheets API sheet ID is not configured.",
                }),
                {status: 500}
            );
        }
